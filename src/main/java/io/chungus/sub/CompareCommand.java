...
public class CompareCommand {
    ...
    public void compare() {
        List<RevCommit> sc = fromBase(sourceCommits);
        Map<String, List<RevCommit>> source = groupByMessage(sc);
        Map<String, List<RevCommit>> target = groupByMessage(fromBase(targetCommits));
        List<String> messages = sc.stream().map(RevCommit::getShortMessage).distinct().toList();
